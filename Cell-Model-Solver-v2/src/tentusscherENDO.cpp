...
void TenTusscherENDO::initConst (double CONSTANTS[], double RATES[], double STATES[])
{
    STATES[0] = -86.709;        // V        FE
    STATES[1] = 138.4;         // Ki       FE 
    STATES[2] = 10.355;         // Nai      FE
    STATES[3] = 0.00013;       // Cai      FE
    STATES[4] = 0.00448;         // Xr1      RL
    STATES[5] = 0.476;          // Xr2      RL
    STATES[6] = 0.0087;         // Xs       RL
    STATES[7] = 0.00155;        // m        RL
    STATES[8] = 0.7573;          // h        RL
    STATES[9] = 0.7225;         // j        RL
    STATES[10] = 0.00036;       // Cass     FE
    STATES[11] = 3.164e-5;      // d        RL
    STATES[12] = 0.8009;        // f        RL
    STATES[13] = 0.9778;        // f2       RL
    STATES[14] = 0.9953;        // fCass    RL
    STATES[15] = 0.3212;      // s        RL
    STATES[16] = 2.235e-8;      // r        RL
    STATES[17] = 3.715;         // CaSR     FE
    STATES[18] = 0.9068;        // Rprime   FE
...
void TenTusscherENDO::compRates (double t, double CONSTANTS[], double RATES[], double STATES[], double ALGEBRAIC[])
{
...
void TenTusscherENDO::compVariables(double t, double* CONSTANTS, double* RATES, double* STATES, double* ALGEBRAIC)
{
    ...
    ALGEBRAIC[12] = (t -  floor(t/CONSTANTS[6])*CONSTANTS[6]>=CONSTANTS[5]&&t -  floor(t/CONSTANTS[6])*CONSTANTS[6]<=CONSTANTS[5]+CONSTANTS[7] ? - CONSTANTS[8] : 0.00000);
...
void TenTusscherENDO::compRates_FE (double t, double CONSTANTS[], double RATES[], double STATES[], double ALGEBRAIC[])
{
...
void TenTusscherENDO::RL (double DT, double CONSTANTS[], double RATES[], double STATES[], double ALGEBRAIC[])
{
    // Forward Euler for V, Ki, Nai, Cai
    STATES[0] = STATES[0] + RATES[0]*DT;
    STATES[1] = STATES[1] + RATES[1]*DT;
    STATES[2] = STATES[2] + RATES[2]*DT;
    STATES[3] = STATES[3] + RATES[3]*DT;

    // Rush-Larsen for Xr1, Xr2, Xs, m, h, j
    STATES[4] = ALGEBRAIC[0] + (STATES[4] - ALGEBRAIC[0])*exp(-DT/ALGEBRAIC[34]);
    STATES[5] = ALGEBRAIC[1] + (STATES[5] - ALGEBRAIC[1])*exp(-DT/ALGEBRAIC[35]);
    STATES[6] = ALGEBRAIC[2] + (STATES[6] - ALGEBRAIC[2])*exp(-DT/ALGEBRAIC[36]);
    STATES[7] = ALGEBRAIC[3] + (STATES[7] - ALGEBRAIC[3])*exp(-DT/ALGEBRAIC[37]);
    STATES[8] = ALGEBRAIC[4] + (STATES[8] - ALGEBRAIC[4])*exp(-DT/ALGEBRAIC[38]);
    STATES[9] = ALGEBRAIC[5] + (STATES[9] - ALGEBRAIC[5])*exp(-DT/ALGEBRAIC[39]);
    
    // Forward Euler for Cass
    STATES[10] = STATES[10] + RATES[10]*DT;

    // Rush-Larsen for d, f, f2, fCass, s, r
    STATES[11] = ALGEBRAIC[6] + (STATES[11] - ALGEBRAIC[6])*exp(-DT/ALGEBRAIC[42]);
    STATES[12] = ALGEBRAIC[7] + (STATES[12] - ALGEBRAIC[7])*exp(-DT/ALGEBRAIC[20]);
    STATES[13] = ALGEBRAIC[8] + (STATES[13] - ALGEBRAIC[8])*exp(-DT/ALGEBRAIC[21]);
    STATES[14] = ALGEBRAIC[9] + (STATES[14] - ALGEBRAIC[9])*exp(-DT/ALGEBRAIC[22]);
    STATES[15] = ALGEBRAIC[10] + (STATES[15] - ALGEBRAIC[10])*exp(-DT/ALGEBRAIC[23]);
    STATES[16] = ALGEBRAIC[11] + (STATES[16] - ALGEBRAIC[11])*exp(-DT/ALGEBRAIC[24]);

    // Forward Euler for CaSR, Rprime
