...
void solve_monodomain (struct monodomain_solver *the_monodomain_solver, struct ode_solver *the_ode_solver,
    ...
    */
    ...
    // Using LU Decomposition to build the matrix
    the_monodomain_solver->A = (double*)malloc(sizeof(double)*the_grid->num_active_cells*the_grid->num_active_cells);
    the_monodomain_solver->x = (double*)malloc(sizeof(double)*the_grid->num_active_cells);
    the_monodomain_solver->b = (double*)malloc(sizeof(double)*the_grid->num_active_cells);
    the_monodomain_solver->pivot = (int*)malloc(sizeof(int)*the_grid->num_active_cells);
    
    assembly_matrix_custom(the_monodomain_solver,the_grid);
    lu_decomposition(the_monodomain_solver,the_grid);
    ...
    while (cur_time <= finalT) 
    {
        ...
        // Solving linear system using back-and-forward substitution
        back_forward_substitution(the_monodomain_solver,the_grid);
        move_solution_to_grid(the_monodomain_solver,the_grid);
...
void assembly_matrix_custom (struct monodomain_solver *monodomain_solver, struct grid *grid)
{
    double beta = monodomain_solver->beta;
    double cm = monodomain_solver->cm;
    double dt = monodomain_solver->dt;
    double sigma = monodomain_solver->sigma_x;
    struct cell_node **ac = grid->active_cells;
    
    double *A = monodomain_solver->A;
    struct graph *pk = grid->the_purkinje_network;
    int n = grid->num_active_cells;

    // Build the matrix
    struct node *ptr = pk->list_nodes;
    for (int i = 0; i < n; i++, ptr = ptr->next)
    {
        double h = ac[i]->face_length;
        int u = ptr->id;
        struct edge *ptrl = ptr->list_edges;
        double value = ALPHA(beta,cm,dt,h);
...
void choose_pivot (double *A, int *pivot_line, double *Amax, const int i, const int N)
{
    *pivot_line = i;
    *Amax = fabs(A[i*N+i]);
...
void switch_lines (double *A, int pivot[], const int pivot_line, const int i, const int N)
{
    // Copy the role line
    ...
    {
        double tmp = A[i*N+j];
        A[i*N+j] = A[pivot_line*N+j];
        A[pivot_line*N+j] = tmp;
    }
    int m = pivot[i];
...
void lu_decomposition (struct monodomain_solver *monodomain_solver, struct grid *grid)
{
    double *A = monodomain_solver->A;
    int N = grid->num_active_cells;
    // Auxiliary vector y and pivot, they can be static arrays ...
    int *pivot = monodomain_solver->pivot;
    ...
    for (int i = 0; i < N-1; i++)
    {
        choose_pivot(A,&pivot_line,&Amax,i,N);
        // The pivot element changed ? If so we need to switch lines
        if (pivot_line != i)
        {
            switch_lines(A,pivot,pivot_line,i,N);
        }
...
void back_forward_substitution (struct monodomain_solver *monodomain_solver, struct grid *grid)
{
    int N = grid->num_active_cells;
    double *A = monodomain_solver->A;
    double *b = monodomain_solver->b;
    double *x = monodomain_solver->x;
    int *pivot = monodomain_solver->pivot;
    double y[N];
...
void move_solution_to_grid (struct monodomain_solver *monodomain_solver, struct grid *grid)
{
    int N = grid->num_active_cells;
    double *x = monodomain_solver->x;
    struct cell_node **ac = grid->active_cells;

    // Copy the solution of the system into the cells of the grid
    for (int i = 0; i < N; i++)
    {
        ac[i]->v = x[i];
    }
