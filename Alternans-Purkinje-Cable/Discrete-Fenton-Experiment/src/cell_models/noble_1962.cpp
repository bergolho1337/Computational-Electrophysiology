...
void RHS_cpu(const double *sv, double *rDY_, double stim_current)
{
    ...
    double i_na =  (g_na+1.4e-01)*(V_old_ - E_na);
    ...
    double g_K1 = (((1.2*exp((((-V_old_)-9.0e+01)/5.0e+01)))+(1.5e-02*exp(((V_old_+9.0e+01)/6.0e+01)))));
    double g_K2 =  1.2f*pow(n_old_, 4.00000);
    double i_k =  (g_K1+g_K2)*(V_old_+100.000);
    double i_leak =  g_L*(V_old_ - E_L);
