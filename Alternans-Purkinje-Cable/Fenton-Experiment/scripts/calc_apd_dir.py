...
def main():
    ...
    if (len(sys.argv) != 3):
        print("Usage:> %s <input_dir> <output_filename>" % (sys.argv[0]))
        sys.exit(1)
    else:
        dir_name = sys.argv[1]
        ...
        for f in files:
            ...
            #print("Even APD = %.10lf" % even_apd)
            #print("Odd APD = %.10lf" % odd_apd)   
            #print("---------------------------------------------------------")
    

        cell_ids, even_apds, odd_apds = sort_apds_by_cell_id(cell_ids,even_apds,odd_apds)
        plot_apd_over_cable(cell_ids,even_apds,odd_apds,dir_name,output_filename)
