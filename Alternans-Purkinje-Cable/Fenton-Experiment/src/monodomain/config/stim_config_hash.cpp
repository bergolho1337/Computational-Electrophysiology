...
struct stim_config_hash *internal_stim_config_hash_create(size_t size) {
    ...
    d = (struct stim_config_hash *)malloc (sizeof (*d));

    assert (d != 0);

    d->size = size;
    d->n = 0;
    d->table = (struct stim_config_elt **)malloc (sizeof (struct stim_config_elt *) * d->size);
...
void stim_config_hash_insert (struct stim_config_hash *d, const char *key,  struct stim_config *value) {
    ...
    e = (struct stim_config_elt *) malloc (sizeof (*e));
