                        ...
                        struct user_options *options)
{
    ...
    for (int k = 0; k < M; k++)
    {
        ...
        update_monodomain(the_ode_solver,x);
...
void solve_all_volumes_odes (struct ode_solver *the_ode_solver, double *vstar, const double dt, const double cur_time)
{
    ...
    solve_model_odes_cpu(the_ode_solver->volumes,merged_stims,dt,n_cells);
...
{
    struct cell_data *volumes = the_ode_solver->volumes;
    int n_cells = the_ode_solver->n_active_cells;

    for (int i = 0; i < n_cells; i++)
        swap(&volumes[i].yOld,&volumes[i].yNew);
}
